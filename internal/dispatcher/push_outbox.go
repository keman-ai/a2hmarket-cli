...
func dispatchDualChannel(es *store.EventStore, row store.PushOutboxRow, session *openclaw.Session, channel, target string, cfg PushDispatchConfig) {
	...
	// Step 2: Send to AI session.
	// When the session has a deliverable channel, use deliver=true via the "agent"
	// RPC so the AI's reply is routed to the external channel (e.g. Feishu).
